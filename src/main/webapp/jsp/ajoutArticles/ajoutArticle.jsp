...
<a href="<c:url value="/AjoutGamme"/>" class="btn btn-primary">Nouvelle gamme de gourde</a>
...
<div>
    <form method="post" action="AjoutArticle">
        ...
        <select class="form-select" aria-label="Default select example" name="gamme">
            ...
            <c:forEach var="gamme" items="${listeGammes}">
                <option value="<c:out value="${gamme.getId_gamme()}"/>"> <c:out value="${gamme.getGamme()}"/> </option>
            </c:forEach>
        </select>
